...
const createTaiKhaoThi = (newTaiKhaoThi) => {
    return new Promise(async (resolve, reject) => {
        const { TaiKhaoThiId, QuanNhanId, ThoiDiem, Quy, Nam, HocKy, HinhThucKhaoThi, MaLopHocPhan, MonHoc, KhoiLuongCongViec, SoGioQuyDoi, TrangThai, edituser, edittime, GhiChu } = newTaiKhaoThi
        try {
            ...
            const newTaiKhaoThi = await TaiKhaoThi.create({
                TaiKhaoThiId, QuanNhanId, ThoiDiem, Quy, Nam, HocKy, HinhThucKhaoThi, MaLopHocPhan, MonHoc, KhoiLuongCongViec, SoGioQuyDoi, TrangThai, edituser, edittime, GhiChu
            })
            if (newTaiKhaoThi) {
                resolve({
                    status: 'OK',
...
const getTaiKhaoThiByQuanNhanId = (id) => {
    return new Promise(async (resolve, reject) => {
        try {
            const quaTrinhCongTacList = await TaiKhaoThi.find({
                QuanNhanId: id
            });
